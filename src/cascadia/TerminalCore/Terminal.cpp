...
void Terminal::Create(til::point viewportSize, til::CoordType scrollbackLines, IRenderTarget& renderTarget)
{
    _mutableViewport = Viewport::FromDimensions({ 0, 0 }, viewportSize);
    _scrollbackLines = scrollbackLines;
    const til::point bufferSize{ viewportSize.x,
...
{
    const til::point viewportSize{ Utils::ClampToShortMax(settings.InitialCols(), 1),
...
[[nodiscard]] HRESULT Terminal::UserResize(const til::point viewportSize) noexcept
{
    ...
    const auto dx = ::base::ClampSub(viewportSize.x, oldDimensions.width);
    const auto newBufferHeight = ::base::ClampAdd(viewportSize.y, _scrollbackLines);

    til::point bufferSize{ viewportSize.x, newBufferHeight };
...
#pragma warning(pop)
    ...
    const til::CoordType proposedTopFromLastLine = base::MakeClampedNum(maxRow) - viewportSize.y + 1;
