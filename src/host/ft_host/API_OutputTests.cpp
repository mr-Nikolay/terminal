...
void OutputTests::WriteConsoleOutputWWithClipping()
{
    ...
    expectedRegion.Left = std::max(0, adjustedRegion.Left);
    expectedRegion.Top = std::max(0, adjustedRegion.Top);
    expectedRegion.Right = std::min(bufferSize.X - 1, adjustedRegion.Right);
    expectedRegion.Bottom = std::min(bufferSize.Y - 1, adjustedRegion.Bottom);
